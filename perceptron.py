...
class Perceptron:
    ...
    def fit(self, X, y):
        ...
        for epoch in np.arange(0, self.epochs):
            ...
            for (x, target) in zip(X, y):
                # take the dot product between the feature matrix and the weight matrix
                # then pass the value through the step function to obtain the prediction
                p = self.step(np.dot(x, self.W))

                # only perform a weight update if ur prediction does not match the target
                if p != target:
                    # determine the error
                    error = p - target
    ...
    def predict(self, X, addBias=True):
        ...
        # check to see if the bias column should be addes
        if addBias:
            # insert a column of ones as the last entry in the feature matrix
            # this little trick allows us to treat the bias as a trainable parameter within the weight matrix
            X = np.c_[X, np.ones((X.shape[0]))]
